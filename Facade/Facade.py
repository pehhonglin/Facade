...
p_home = [0.2,-0.23,0.6]            # x , y , z coordinates of end effector relative to base frame at home position
...
def foo(q):
    ...
    while True:
        if not q.empty():
            ...
            print(p1)
            string6=('movel(p[' + str(p_home[0]) +',' + str(p_home[1]) + ',' + str(p_home[2]) + ',' + str(row1) + ',' + str(pitch1) +',' + str(yaw1) + '],a=' + a + ',v=' + v + ')\n')
            s.send (string6.encode())
            time.sleep(1)
            print("done")
